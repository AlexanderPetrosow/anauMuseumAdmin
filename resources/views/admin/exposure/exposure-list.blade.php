...
@section('title', __('words.exposure-list-pg'))
...
<form method="POST" action="/{{$lang}}/admin/delete-exposure/{{count($exposures)}}">
  @csrf
<div class="demo-inline-spacing d-flex justify-content-end">
  <a href="/{{$lang}}/admin/qr-all-exposure">
  ...
  <a href="/{{$lang}}/admin/add-exposure">
  ...
  <div class="col-lg-12 card mt-3 py-3 h-75">
    <table class="table">
      ...
      @for($i=0; $i < count($exposures); $i++)
        <tr class="@if(count($exposures) <= ($i+1)) border-transparent @endif">
          {{-- <th><input class="form-check-input me-1" type="checkbox" name="check{{$i}}" value="{{$exposures[$i]['id']}}"></th> --}}
          <td>{{$exposures[$i][$lang.'_name']}}</td>
          @if(strlen($exposures[$i][$lang.'_description']) > 100)
          <td>{{mb_substr($exposures[$i][$lang.'_description'],0,100).'...'}}</td>
          @else
          <td>{{$exposures[$i][$lang.'_description']}}</td>
          @endif
          <td>{{$halls_name[$i]}}</td>
          <!-- <td>{{$exposures[$i]['sort']}}</td> -->
          @if($exposures[$i]['status'] == 1)
          <td>{{__('words.turn-on')}}</td>
          @else
          <td>{{__('words.turn-off')}}</td>
          @endif
          <td>{{$exposures[$i]['updated_at']}}</td>
          <td>{{$exposures[$i]['created_at']}}</td>
          <td class="text-center">
            <a href="/{{$lang}}/admin/edit-exposure/{{$exposures[$i]['id']}}">
          ...
          <td class="text-center">
            <a href="/{{$lang}}/admin/qr-exposure/{{$exposures[$i]['id']}}">
              <button type="button" class="btn btn-icon">
...
<!-- <div class="col-lg-12 card mt-3 p-0">
  <div class="card align-bottom">
    <h5 class="card-header">Удалить экспозицию</h5>
    <div class="card-body">
      <div class="mb-3 col-12 mb-0">
        <div class="alert alert-warning">
          <h6 class="alert-heading fw-bold mb-1">Вы уверены, что хотите удалить экспозицию?</h6>
          <p class="mb-0">Как только вы удалите экспозицию, пути назад уже не будет. Пожалуйста, будьте уверены.</p>
        </div>
      </div>
      <form id="formAccountDeactivation" onsubmit="return false">
        <button type="submit" class="btn btn-danger deactivate-account">Удалить экспозицию</button>
      </form>
    </div>
  </div>
</div> -->
