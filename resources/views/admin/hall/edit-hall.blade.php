...
@section('title', __('words.edit-hall-pg'))
...
<h4 class="fw-bold py-3 mb-4">
  <span class="text-muted fw-light">{{__('words.halls')}} /</span> {{__('words.edit-hall')}}
</h4>
...
<div class="row">
  <div class="col-md-6">
    <div class="card mb-4">
      ...
      <hr class="my-0">
      <div class="card-body">
        <form action="/{{$lang}}/admin/edit-hall/{{$hall[0]['id']}}" method="POST">
        @csrf
          <div class="row">
            <div class="mb-1 col-lg-12 col-md-6">              
              <label for="firstName" class="form-label">{{__('words.rename')}}</label>
              <div class="input-group">
              <input class="form-control" type="text" id="firstName" name="tm_name" placeholder="{{__('words.write-name')}}" value="{{ $hall[0]['tm_name'] }}" autofocus />
            ...
            <span class="text-danger">@error('tm_name'){{$message}}@enderror<span>
              ...
              <div class="mb-1 col-lg-12 col-md-6">              
                ...
                <div class="input-group">
                <input class="form-control" type="text" id="firstName" name="ru_name" placeholder="{{__('words.write-name')}}" value="{{ $hall[0]['ru_name'] }}" autofocus />
                <span class="input-group-btn">
            ...
            <div class="mb-3  col-lg-12 col-md-6">              
              <!-- <label for="firstName" class="form-label">Наименование</label> -->
              <div class="input-group">
              <input class="form-control" type="text" id="firstName" name="eng_name" placeholder="{{__('words.write-name')}}" value="{{ $hall[0]['eng_name'] }}" autofocus />
              <span class="input-group-btn">
            ...
            <label for="firstName" class="form-label">{{__('words.status')}}</label>
                  <select name="status" id="input-status" class="form-control">
                    <option value="1" <?php echo $hall[0]['status'] == 1 ? 'selected' : ''; ?>>{{__('words.turn-on')}}</option>
                    <option value="0" <?php echo $hall[0]['status'] == 0 ? 'selected' : ''; ?>>{{__('words.turn-off')}}</option>
                  </select>
                </div>
          <div class="mt-5">
