...
@section('title', __('words.add-user-pg'))
...
<h4 class="fw-bold py-3 mb-4">
  <span class="text-muted fw-light">{{__('words.users')}} /</span> {{__('words.add-user')}}
</h4>
...
<div class="row">
  <div class="col-md-6">
    <div class="card mb-4">
      ...
      <hr class="my-0">
      <div class="card-body">
        <form action="/{{$lang}}/admin/add-user" method="POST">
        @csrf
          <div class="row">
            <div class="mb-3 col-12">              
              <label for="firstName" class="form-label">{{__('words.login')}}</label>
              <div class="input-group">
              <input class="form-control" type="text" id="firstName" name="login" placeholder="admin-maral / kassir-maral / content-maral" autofocus />
            ...
            <div class="mb-3 col-12">              
              ...
              <div class="input-group">
              <input class="form-control" type="text" id="firstName" name="name" placeholder="Maral Myradova" autofocus />
              <span class="input-group-btn">
              </div>
              <span class="text-danger">@error('name'){{$message}}@enderror<span>
            </div>
            <div class="mb-3 form-password-toggle">
                <label class="form-label" for="basic-default-password32">{{__('words.password')}}</label>
            ...
            <div class="mb-3 col-md-12">
              ...
              <select id="language" name="group" class="select2 form-select">
                <option value="1" selected>{{__('words.admin')}}</option>
                <option value="2">{{__('words.casher')}}</option>
                <option value="3">{{__('words.content')}}</option>
              </select>
            </div>
            <div class="mb-3 col-md-12">
              <label for="language" class="form-label">{{__('words.status')}}</label>
              <select id="language" name="status" class="select2 form-select">
                <option value="1" selected>{{__('words.turn-on')}}</option>
